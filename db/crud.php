...
use LDAP\Result;
    ...
    class crud{
        ...
        public function insertRegistrants($fname, $lname, $dob, $email, $contact, $gender, $avatar_path)
        {
            try{
                $sql = "INSERT INTO registrant (firstname,lastname,dob,emailaddress,contactnum,gender_id,avatar_path) VALUES (:fname, :lname, :dob, :email, :contact, :gender, :avatar_path)";
        ...
        public function editregistrant($id, $fname, $lname, $dob, $email, $contact, $gender)
            {
                try{
                $sql = "UPDATE `registrant` SET `firstname`=:fname,`lastname`=:lname,`dob`=:dob,`emailaddress`=:email,`contactnum`=:contact,`gender_id`=:gender WHERE registrant_id=:id";   
                $stmt = $this->db->prepare($sql);
